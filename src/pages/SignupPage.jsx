...
const SignupPage = () => {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      console.error('Firebase Google Signup Error in Page:', authError.message);
      if (authError.code === 'auth/popup-closed-by-user') { setError('Google sign-in popup was closed before completion.'); }
      else if (authError.code === 'auth/account-exists-with-different-credential') { setError('An account already exists with this email address using a different sign-in method.'); }
      else { setError(authError.message || 'Failed to sign up/in with Google.'); }
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 flex justify-center items-start min-h-[calc(100vh-8rem)]">
      <div className="w-full max-w-md">
        <div className="bg-white dark:bg-gray-800 shadow-2xl rounded-xl p-8 md:p-10">
          <h1 className="text-3xl font-bold text-center text-gray-900 dark:text-white mb-8"> Create Your FolioXe Account </h1>
          ...
          <button type="button" onClick={handleGoogleSignup} className="w-full flex items-center justify-center bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 font-medium py-3 px-4 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 focus:ring-blue-500 disabled:opacity-70" disabled={loading}> <GoogleIcon /> Sign Up with Google </button>
