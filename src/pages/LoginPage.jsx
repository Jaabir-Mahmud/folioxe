...
const LoginPage = () => {
  ...
  const [formLoading, setFormLoading] = useState(false); // Renamed to avoid confusion with authLoading
  const [showResendVerification, setShowResendVerification] = useState(false);

  const { login, loginWithGoogle, resendVerificationEmail, isAuthenticated, loading: authLoading } = useAuth(); // Get isAuthenticated and authLoading
  const navigate = useNavigate();
  const location = useLocation();
  const from = location.state?.from?.pathname || "/profile"; // Default to /profile if logged in, or from previous location

  // Redirect if user is already authenticated
  if (authLoading) {
    // Auth context is still loading, render minimally or a spinner
    return <div className="flex justify-center items-center min-h-[calc(100vh-10rem)]"><p>Loading...</p></div>; 
  }

  if (isAuthenticated) {
    // User is already logged in, redirect them from the login page
    return <Navigate to={from === "/login" ? "/profile" : from} replace />; 
    // If 'from' was '/login' (e.g. direct navigation), go to profile, else go to intended 'from'
  }
        ...
        <div className="bg-white dark:bg-gray-800 shadow-2xl rounded-xl p-8 md:p-10">
          ...
          <form onSubmit={handleEmailLogin}>
            ...
            <div className="mb-6">
              ...
              <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 dark:bg-gray-700 dark:text-white transition-colors" placeholder="you@example.com" required disabled={formLoading} />
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} id="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-2.5 pr-10 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 dark:bg-gray-700 dark:text-white transition-colors" placeholder="••••••••" required disabled={formLoading} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 px-3 flex items-center text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200" aria-label={showPassword ? "Hide password" : "Show password"}>
                  {showPassword ? <EyeSlashIcon /> : <EyeIcon />}
                </button>
              </div>
            </div>
            {/* Resend verification button */}
            {showResendVerification && ( <div className="mb-4 text-center"> <button type="button" onClick={handleResendVerification} className="text-sm text-blue-600 dark:text-blue-400 hover:underline disabled:opacity-70" disabled={formLoading} > {formLoading ? 'Sending...' : 'Resend verification email'} </button> </div> )}
            {/* Login button */}
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white font-semibold py-3 px-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 disabled:opacity-70" disabled={formLoading}>
              {formLoading ? 'Logging In...' : 'Log In'}
            </button>
          </form>
          {/* ... (OR separator, Google Login, Sign up link) ... */}
